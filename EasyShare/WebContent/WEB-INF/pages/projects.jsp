        ...
        <div id="page-wrapper">
		  <div class="header">
                        <h1 class="page-header">
                            现役项目 <small>你好 ${user.commonUserInfo.loginName}</small>
                        </h1>
						<ol class="breadcrumb">
					  ...
					  <li class="active">现役项目</li>
					</ol>

		</div>
            <div id="page-inner">
			<c:forEach items="${AllAliveProjects}" var="project">
			<c:if test="${project.status==1}">
                <div class="row">
                    <div class="col-md-12">
                        <div class="jumbotron" style="font-size: 16px;">
                            <h2>${project.projectName}</h2>
                            <a href="${pageContext.request.contextPath}/projects.html?type=done&id=${project.projectID}&page=allPro"><button type="button" class="buttoner">项目完成</button></a>
                            <br>
                                       ...
                                       <table class="table table-hover">
                                           <tbody>
                                               ...
                                               <tr>
                                                   <td>立项日期:</td>
                                                   <c:choose>
                                                   	<c:when test="${empty project.startDate}"><td>未填写</td></c:when>
                                                   	<c:otherwise><td>${project.startDate}</td></c:otherwise>
                                                   </c:choose>
                                               </tr>
                                               <tr>
                                                   <td>结题日期:</td>
                                                   <c:choose>
                                                   	<c:when test="${empty project.endDate}"><td>未填写</td></c:when>
                                                   	<c:otherwise><td>${project.endDate}</td></c:otherwise>
                                                   </c:choose>
                                               </tr>
                                               <tr>
                                                   <td>项目负责人:</td>
                                                   <td>${project.commonUserInfo.loginName}</td>
                                               </tr>
                                               <tr>
                                                   <td>是否签订合同:</td>
                                                   <c:choose>
                                                   	<c:when test="${project.isContract==1}"><td>未签</td></c:when>
                                                   	<c:otherwise><td>已签</td></c:otherwise>
                                                   </c:choose>
                                               </tr>
                                               <tr>
                                                   <td>日志时间轴:</td>
                                                   <td><a href="${pageContext.request.contextPath}/historyTimeLine.html?type=init&id=${project.projectID}" class="linkFont" target="_blank">查看</a></td>
                                               </tr>
                                               <tr>
                                                   <td>项目书:</td>
